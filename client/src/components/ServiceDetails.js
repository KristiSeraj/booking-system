                ...
                <div className="bg-white shadow-md rounded-lg p-6">
                    ...
                    <div className="mt-8">
                        <div className="mt-8 flex items-center justify-between mb-6">
                            ...
                            {user?.role === 'provider' && <CreateSlotModal onSave={handleCreate}/>}
                        </div>
                            ...
                            <ul className="space-y-4">
                                {availableSlots.map((slot) => (
                                    <li key={slot._id}
                                        ...
                                        {user?.role === 'provider' ? <ThreeDotMenu onEdit={handleEdit} onDelete={handleDelete} /> : <button onClick={() => handleBookAppoinment(service._id, slot._id)}>Book</button>}
                                    </li>
                                ))}
                            </ul>
                        ) : (
